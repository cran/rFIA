...
tpaStarter_test <- function(x,
  ...
  suppressWarnings({
    ...
    if (Sys.info()['sysname'] == 'Windows'){
      ...
      out <- parLapply(cl, X = names(plts), fun = tpaHelper1_test, plts, db, grpBy, aGrpBy, byPlot)
      #stopCluster(cl) # Keep the cluster active for the next run
    } else { # Unix systems
      out <- mclapply(names(plts), FUN = tpaHelper1_test, plts, db, grpBy, aGrpBy, byPlot, mc.cores = nCores)
    }
  })
  ...
  if (byPlot){
    ...
    suppressWarnings({
      ...
      if (Sys.info()['sysname'] == 'Windows'){
        ...
        #   library(rFIA)
        # })
        out <- parLapply(cl, X = names(popState), fun = tpaHelper2_test, popState, a, t, grpBy, aGrpBy, method)
        stopCluster(cl)
      } else { # Unix systems
        out <- mclapply(names(popState), FUN = tpaHelper2_test, popState, a, t, grpBy, aGrpBy, method, mc.cores = nCores)
      }
    })
    ## back to dataframes
    out <- unlist(out, recursive = FALSE)
...
tpa_test <- function(db,
  ...
  ## Run the main portion
  out <- lapply(X = iter, FUN = tpaStarter_test, db,
                grpBy_quo = grpBy_quo, polys, returnSpatial,
                bySpecies, bySizeClass,
                landType, treeType, method,
                lambda, treeDomain, areaDomain,
