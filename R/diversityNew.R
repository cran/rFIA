diversityStarter <- function(x,
  ...
  db$COND <- select(db$COND, c('PLT_CN', 'CONDPROP_UNADJ', 'PROP_BASIS', 'COND_STATUS_CD', 'CONDID', all_of(grpC), 'aD_c', 'landD'))
  db$TREE <- db$TREE %>%
    mutate(TRE_CN = CN) %>%
    select(c('PLT_CN', 'TRE_CN', 'CONDID', 'DIA', 'grp', 'state', 'SUBP', 'TREE', all_of(grpT), 'tD', 'typeD'))
...
diversity_backup <- function(db,
  ...
  if (byPlot){
    ...
    if (totals) {
      tOut <- tOut %>%
        select(grpBy, H_a, Eh_a, S_a, H_b, Eh_b, S_b, H_g, Eh_g, S_g, AREA_TOTAL, H_a_SE, Eh_a_SE, S_a_SE, AREA_TOTAL_SE, nPlots)

    } else {
      tOut <- tOut %>%
        select(grpBy, H_a, Eh_a, S_a, H_b, Eh_b, S_b, H_g, Eh_g, S_g, H_a_SE, Eh_a_SE, S_a_SE, nPlots)
    }
