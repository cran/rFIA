invasiveStarter <- function(x,
  ...
  reqTables <- c('PLOT', 'INVASIVE_SUBPLOT_SPP', 'SUBP_COND', 'COND', 'POP_PLOT_STRATUM_ASSGN', 'POP_ESTN_UNIT', 'POP_EVAL',
  ...
  db$PLOT <- filter(db$PLOT, INVASIVE_SAMPLING_STATUS_CD %in% 1:2)
  ...
  db$INVASIVE_SUBPLOT_SPP <- select(db$INVASIVE_SUBPLOT_SPP, c('PLT_CN', 'COVER_PCT', 'VEG_SPCD', 'SUBP', 'CONDID', 'SYMBOL', 'SCIENTIFIC_NAME', 'COMMON_NAME'))
  #filter(DIA >= 5)
  ...
  if (byPlot){
    ...
    suppressWarnings({
      ...
      if (Sys.info()['sysname'] == 'Windows'){
        ...
        #   library(rFIA)
        # })
        out <- parLapply(cl, X = names(popState), fun = invHelper2, popState, a, t, grpBy, aGrpBy, method)
        stopCluster(cl)
      } else { # Unix systems
        out <- mclapply(names(popState), FUN = invHelper2, popState, a, t, grpBy, aGrpBy, method, mc.cores = nCores)
      }
    })
    ## back to dataframes
    out <- unlist(out, recursive = FALSE)
