vegStructStarter <- function(x,
  ...
  db$PLOT <- dplyr::filter(db$PLOT, P2VEG_SAMPLING_STATUS_CD %in% 1:3)
  ...
  db$SUBP_COND <- dplyr::select(db$SUBP_COND, c(PLT_CN, SUBP, CONDID, SUBPCOND_PROP)) %>%
    ## Drop visits not used in our eval of interest
    dplyr::filter(PLT_CN %in% pops$PLT_CN)

  db$P2VEG_SUBP_STRUCTURE <- dplyr::select(db$P2VEG_SUBP_STRUCTURE,
