...
diMort <- function(db,
  ...
  db$POP_EVAL  <- db$POP_EVAL %>%
    ...
    filter(EVAL_TYP %in% c('EXPGROW', 'EXPMORT', 'EXPREMV') & GROWTH_ACCT == 'Y') %>%
  ...
  suppressWarnings({
    ...
    if (Sys.info()['sysname'] == 'Windows'){
      ...
      out <- parLapply(cl, X = names(plts), fun = diMortHelper1, plts, db, grpBy, aGrpBy, byPlot, minLive)
      #stopCluster(cl) # Keep the cluster active for the next run
    } else { # Unix systems
      out <- mclapply(names(plts), FUN = diMortHelper1, plts, db, grpBy, aGrpBy, byPlot, minLive, mc.cores = nCores)
    }
  })
  ...
  if (byPlot){
    ...
    suppressWarnings({
      ...
      if (Sys.info()['sysname'] == 'Windows'){
        ...
        #   library(rFIA)
        # })
        out <- parLapply(cl, X = names(popState), fun = diMortHelper2, popState, a, t, grpBy, aGrpBy, method)
        stopCluster(cl)
      } else { # Unix systems
        out <- mclapply(names(popState), FUN = diMortHelper2, popState, a, t, grpBy, aGrpBy, method, mc.cores = nCores)
      }
    })
    ## back to dataframes
    out <- unlist(out, recursive = FALSE)
...
diMort_old <-  function(db,
  ...
  suppressWarnings({
    ...
    if (Sys.info()['sysname'] == 'Windows'){
      ...
      out <- parLapply(cl, X = names(plts), fun = diMortHelper1, plts, db, grpBy, byPlot, minLive)
      #stopCluster(cl) # Keep the cluster active for the next run
    } else { # Unix systems
      out <- mclapply(names(plts), FUN = diMortHelper1, plts, db, grpBy, byPlot, minLive, mc.cores = nCores)
    }
  })
  ...
  if (byPlot){
    ...
    suppressWarnings({
      ...
      if (Sys.info()['sysname'] == 'Windows'){
        ...
        #   library(rFIA)
        # })
        out <- parLapply(cl, X = names(popState), fun = diMortHelper2, popState, t, grpBy, method)
        stopCluster(cl)
      } else { # Unix systems
        out <- mclapply(names(popState), FUN = diMortHelper2, popState, t, grpBy, method,  mc.cores = nCores)
      }
    })
    ## back to dataframes
    out <- unlist(out, recursive = FALSE)
    ...
    if (str_to_upper(method) %in% c("SMA", 'EMA', 'LMA')){
      ...
      ### Applying the weights
      tEst <- tEst %>%
        mutate_at(vars(aEst:mEst), ~(.*wgt)) %>%
        mutate_at(vars(aVar:cvEst_m), ~(.*(wgt^2))) %>%
        group_by(ESTN_UNIT_CN, .dots = grpBy) %>%
        summarize_at(vars(aEst:plotIn_AREA), sum, na.rm = TRUE)
    ...
    suppressWarnings({
      tOut <- tEst %>%
        group_by(.dots = grpBy) %>%
        summarize_all(sum,na.rm = TRUE) %>%
        mutate(MORT_RATE = qEst / aEst,
               RECR_RATE = pEst / aEst,
               LAMBDA = sEst / aEst,
               BAA_RATE = bEst / aEst,
               SUST_INDEX = mEst / aEst,
               AREA_TOTAL = aEst,
               ## Ratio variance
               qVar = (1/AREA_TOTAL^2) * (qVar + (MORT_RATE^2 * aVar) - 2 * MORT_RATE * cvEst_q),
               pVar = (1/AREA_TOTAL^2) * (pVar + (RECR_RATE^2 * aVar) - 2 * RECR_RATE * cvEst_p),
               sVar = (1/AREA_TOTAL^2) * (sVar + (LAMBDA^2 * aVar) - 2 * LAMBDA * cvEst_s),
               bVar = (1/AREA_TOTAL^2) * (bVar + (BAA_RATE^2 * aVar) - 2 * BAA_RATE * cvEst_b),
    ...
    if (totals) {
      tOut <- tOut %>%
        select(grpBy, SUST_INDEX, LAMBDA, MORT_RATE, RECR_RATE, BAA_RATE, AREA_TOTAL,
               SUST_INDEX_SE, LAMBDA_SE, MORT_RATE_SE, RECR_RATE_SE, BAA_RATE_SE, AREA_TOTAL_SE,
               nPlots)

    } else {
      tOut <- tOut %>%
        select(grpBy, SUST_INDEX, LAMBDA, MORT_RATE, RECR_RATE, BAA_RATE,
               SUST_INDEX_SE, LAMBDA_SE, MORT_RATE_SE, RECR_RATE_SE, BAA_RATE_SE,
               nPlots)
      }
